...
class Solution {
    public int canShip(int[] weights, int capacity, int sum, int days){
        for(int i : weights){
            sum += i;
            if(sum > capacity){
                ++days;
    ...
    public int shipWithinDays(int[] weights, int days) {
        int low = 0, high = 0, maxi = Integer.MIN_VALUE;
        for(int i : weights){
            high += i;
            maxi = Math.max(maxi, i);
        }
        low = Math.max(high / days, maxi);
        while(low < high){
            int mid = low + (high - low) / 2;
            if(canShip(weights, mid, 0, 1) > days)
                low = mid + 1;
            else high = mid;
        }
        return low;
