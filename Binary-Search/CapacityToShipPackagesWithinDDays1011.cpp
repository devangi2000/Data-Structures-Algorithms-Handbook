...
class Solution {
public:
    int canShip(vector<int> &weights, int capacity, int sum = 0, int days = 1){
        for(auto i : weights){
           sum += i;
           if(sum > capacity){
               sum = i;
    ...
    int shipWithinDays(vector<int>& weights, int days) {
        int high = accumulate(weights.begin(), weights.end(), 0), low = (high/days ,*max_element(weights.begin(), weights.end()));
        while(low < high){
            auto mid = (low + high)/2;
