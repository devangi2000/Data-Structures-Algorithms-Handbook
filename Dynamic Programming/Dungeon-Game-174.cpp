...
//  dp[i][j] = max(1,val)   => if val is negative that means we dont have to spend health on this 
// cell so it should be kept minimum i.e. 1. 
//  So we had to keep both values 1 as instead of max as for calculating the health for princess' 
// cell we only need 1 health minimum.  
